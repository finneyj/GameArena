...
public class GameArena extends JPanel implements Runnable, KeyListener, MouseListener, MouseMotionListener {
	 ...
	 * @param width        The width of the playing area, in pixels.
	 * @param height       The height of the playing area, in pixels.
	 * @param createWindow Defines if a window should be created to host this
	 *                     GameArena. @see getPanel.
	 */
	...
	public void setSize(int width, int height) {
		...
		if (frame != null)
			frame.setSize(arenaWidth + frame.getInsets().left + frame.getInsets().right,
					arenaHeight + frame.getInsets().top + frame.getInsets().bottom);

	}

	/**
	 * Retrieves the JPanel on which this gameArena is drawn, so that it can be
	 * integrated into a users application.
	 * 
	 * n.b. This should only be called if this GameArena was constructed without its
	 * own JFrame
	 * 
	...
	public void paint(Graphics gr) {
		...
		if (!rendered) {
			...
			// Create a buffer the same size of the window, which we can reuse from frame to
			// frame to improve performance.
			buffer = new BufferedImage(arenaWidth, arenaHeight, BufferedImage.TYPE_INT_ARGB);
			graphics = buffer.createGraphics();
			graphics.setRenderingHints(renderingHints);

			// Remember that we've completed this initialisation, so that we don't do it
			// again...
			rendered = true;
		}
			...
			if (!this.exiting) {
				...
				for (Object o : things) {
					if (o instanceof Ball) {
						...
						graphics.fillOval((int) (b.getXPosition() - b.getSize() / 2),
								(int) (b.getYPosition() - b.getSize() / 2), (int) b.getSize(), (int) b.getSize());
					}

					if (o instanceof Rectangle) {
						Rectangle r = (Rectangle) o;
						graphics.setColor(this.getColourFromString(r.getColour()));
						graphics.fillRect((int) r.getXPosition(), (int) r.getYPosition(), (int) r.getWidth(),
								(int) r.getHeight());
					}
	...
	private void addThing(Object o, int layer) {
		...
		synchronized (this) {
			if (things.size() > 100000) {
				...
				// If there are no items in the list with an equivalent or higher layer, append
				// this object to the end of the list.
				if (!added)
					things.add(o);
			}
		}
	...
	/**
	 * Adds a given Ball to the GameArena. Once a Ball is added, it will
	 * automatically appear on the window.
	 *
	...
	/**
	 * Adds a given Rectangle to the GameArena. Once a rectangle is added, it will
	 * automatically appear on the window.
	 *
	...
	/**
	 * Adds a given Line to the GameArena. Once a Line is added, it will
	 * automatically appear on the window.
	 *
	...
	/**
	 * Adds a given Text object to the GameArena. Once a Text object is added, it
	 * will automatically appear on the window.
	 *
	...
	/**
	 * Remove a Rectangle from the GameArena. Once a Rectangle is removed, it will
	 * no longer appear on the window.
	 *
	...
	/**
	 * Remove a Ball from the GameArena. Once a Ball is removed, it will no longer
	...
	/**
	 * Remove a Line from the GameArena. Once a Line is removed, it will no longer
	 * appear on the window.
	 *
	...
	/**
	 * Remove a Text object from the GameArena. Once a Text object is removed, it
	 * will no longer appear on the window.
	 *
	...
	/**
	 * Pause for a 1/50 of a second. This method causes your program to delay for
	 * 1/50th of a second. You'll find this useful if you're trying to animate your
	 * application.
	 *
	 */
	public void pause() {
